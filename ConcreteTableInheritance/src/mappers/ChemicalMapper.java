...
public class ChemicalMapper implements ChemicalMapperInterface {
	...
	public ChemicalDomainObject createChemical(int ID, double moles) throws Exception {
		this.ID = ID;
		this.moles = moles;
	...
	public ChemicalDomainObject findByID(int chemicalID) throws Exception {
		...
		if (!element.equals(null)) {
			cdo = createChemical(element.getID(), element.getMoles());
		} else if (!compound.equals(null)) {
			cdo = createChemical(compound.getCompoundID(), compound.getCompoundMoles());
		} else if (!metal.equals(null)) {
			cdo = createChemical(metal.getID(), metal.getMoles());
		} else if (!acid.equals(null)) {
			cdo = createChemical(acid.getAcidID(), acid.getAcidMoles());
		} else if (!base.equals(null)) {
			cdo = createChemical(base.getID(), base.getMoles());
		}
	...
	public ArrayList<ChemicalDomainObject> findLowChemicals() throws Exception {
		...
		for (ElementDTO e : elements) {
			if (e.getMoles() < 20) {
				lowChemicals.add(createChemical(e.getID(), e.getMoles()));
				elements.remove(e);
			}
		}
		...
		for (AcidDTO a : acids) {
			...
			if (a.getAcidMoles() < totalToDissolve) {
				lowChemicals.add(createChemical(a.getAcidID(), a.getAcidMoles()));
			}
		}
		...
		for (BaseDTO b : bases) {
			if (b.getMoles() < 40) {
				lowChemicals.add(createChemical(b.getID(), b.getMoles()));
			}
		}
