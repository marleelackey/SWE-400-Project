...
//		return new ElementDomainObject(this);
	...
	public ElementDomainObject findByID(int id) throws Exception {
		...
		return createElement(rdg.getID(), rdg.getName(), rdg.getAtomicNumber(), rdg.getAtomicMass(),
				rdg.getMoles());
	}
	...
	public ArrayList<ElementDomainObject> findElementsInRange(double highRange, double lowRange) throws Exception {
		...
		for (ChemicalDTO e : edto) {
			edo.add(createElement(e.getChemicalID(), e.getChemicalName(), e.getChemicalAtomicNumber(), e.getChemicalAtomicMass(), e.getChemicalMoles()));
		}
		return edo;
	}
	...
	public ArrayList<ElementDomainObject> getAllElements() throws Exception {
		...
		for (ChemicalDTO e : elements) {
			list.add(createElement(e.getChemicalID(), e.getChemicalName(), e.getChemicalAtomicNumber(), e.getChemicalAtomicMass(), e.getChemicalMoles()));
		}
		return list;
	}
	...
	public void persist() {
		try {
			...
			if (element == null) {
				ChemicalRDG rdg = new ChemicalRDG(ident, name, atomicNumber, atomicMass, moles);
				rdg.insert();
			} else {
